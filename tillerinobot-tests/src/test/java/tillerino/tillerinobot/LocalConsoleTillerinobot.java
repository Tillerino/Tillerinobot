...
/**
 * The purpose of this class and its main function is to completely mock backend and IRC connection to quickly check out
 * the functionality of Tillerinobot.
 *
 * @author Tillerino
 */
@Slf4j
public class LocalConsoleTillerinobot {
    ...
    /**
     * This method will start a Tillerinobot instance, which communicates via stdout, with a bogus backend and an API
     * server.
     *
     * @param args give a port number to start the API server on that port. Otherwise, a random, free port will be
     *     chosen.
     */
    public static void main(String[] args) throws Exception {
        ...
        URI baseUri = UriBuilder.fromUri("http://localhost/")
                ...
                .build();
        HttpServer apiServer = JdkHttpServerFactory.createHttpServer(
                baseUri, ResourceConfig.forApplication(injector.botApiDefinition()));
            ...
            while (running.get()) {
                ...
                if (line.startsWith("/np ")) {
                    dispatch(new PrivateAction(
                            ...
                            username,
                            System.currentTimeMillis(),
                            "is listening to [http://osu.ppy.sh/b/" + line.substring(4) + " title]"));
                } else if (line.startsWith("/nps ")) {
                    dispatch(new PrivateAction(
                            System.currentTimeMillis(),
                            username,
                            System.currentTimeMillis(),
                            "is listening to [https://osu.ppy.sh/b/" + line.substring(4) + " title]"));
                    ...
                    dispatch(
                            new PrivateMessage(System.currentTimeMillis(), username, System.currentTimeMillis(), line));
                }
            }
            return false;
        }
