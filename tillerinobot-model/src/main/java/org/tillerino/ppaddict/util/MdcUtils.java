...
/**
 * With the {@link MDC} we have two problems that originate from the fact that the MDC is bound to a thread: 1) We want
 * to clean up everything that we put in there so that it doesn't leak into a different context that our thread might be
 * working in later. 2) When we transfer a workload into a different thread using a thread pool or some sort of queue,
 * we need to make sure that our MDC travels with the work instead of sticking to the thread.
 */
public class MdcUtils {
    public interface MdcSnapshot {
        /**
         * Applies the snapshot to the current MDC and returns a closer, which will restore the previous state. Keep the
         * result in a try-with clause.
         */
        MdcAttributes apply();
    ...
    /** Implements a mechanism that cleans up changes to the MDC. See {@link MdcUtils#with(String, Object)}. */
    public static final class MdcAttributes implements QuietCloseable {
        ...
        /**
         * Changes a value in the MDC and stores the original value for clean up in {@link #close()}.
         *
         * @param key the key for {@link MDC#put(String, String)}.
         * @param val the value for {@link MDC#put(String, String)}. This argument is passed to
         *     {@link String#valueOf(Object)}, which turns null into the string literal "null".
         */
    ...
    public static MdcSnapshot getSnapshot() {
        Map<String, String> snapshot =
                Optional.ofNullable(MDC.getCopyOfContextMap()).orElseGet(Collections::emptyMap);
        return new MdcSnapshotImpl(snapshot);
    }

    /**
     * Puts a value in the MDC and returns a closeable to restore the current value in the MDC.
     *
     * @param key the key for {@link MDC#put(String, String)}.
     * @param val the value for {@link MDC#put(String, String)}. This argument is passed to
     *     {@link String#valueOf(Object)}, which turns null into the string literal "null".
     * @return use this as a try-with resource. You can pile on more changes to the MDC with
     *     {@link MdcAttributes#add(String, Object)}.
     */
