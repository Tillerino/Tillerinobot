...
public class TestAppender {
    ...
    public static LogRule rule(Class... targets) {
        Validate.exclusiveBetween(
                0, Integer.MAX_VALUE, targets.length, "Need to specify at least one logger to capture");
    ...
    public static class LogRule implements BeforeEachCallback, AfterEachCallback {
        ...
        private LogRule(Supplier<List<LogCaptor>> initializer) {
        ...
        public List<LogEvent> events() {
            return logCaptors.stream()
                    .flatMap((LogCaptor logCaptor) -> logCaptor.getLogEvents().stream())
                    .toList();
        }
    }
}
