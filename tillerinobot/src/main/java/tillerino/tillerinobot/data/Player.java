...
public class Player {
	...
	public static void updateLastSeen(int userid, Database conn, @MillisSinceEpoch long timestamp) throws SQLException {
		try(PreparedStatement statement = conn.prepare("insert into players (userid, lastseen, agetop50) values (?, ?, ?)"
	...
	public void updateTop50(Database database, long maxAge, OsuApi downloader, Clock clock, ConfigService config) throws SQLException, IOException {
		if(lastupdatetop50 > clock.currentTimeMillis() - maxAge)
			return;
		...
		try(var _ = PhaseTimer.timeTask("updatePlayer");
				PreparedStatement statement = database.prepare("update players set lastupdatetop50 = ?, agetop50 = ? where userid = ?")) {
