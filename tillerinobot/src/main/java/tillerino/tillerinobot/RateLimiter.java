...
/**
 * Rate limiter for the osu! API with multiple urgency levels. It keeps one (bounded) queue per priority, adding 1200
 * permits per second to the queues, highest to lowest priority. It drains the queues in reverse order, which will
 * always keep a buffer for high priority threads. The sum of the capacities of the permit queues is 200, which roughly
 * translates to "1200 requests per minute, with burst capability of up to 200 beyond that", which is the documentation
 * of the rate limit of the API.
 */
@Singleton
@Slf4j
public class RateLimiter {
    ...
    /** Returns and clears the time that this thread has spent blocked waiting for a permit. */
