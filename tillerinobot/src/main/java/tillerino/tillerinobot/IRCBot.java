...
/**
 * The name of this class is rather historical. It implements the basic chat event -> response logic and is the heart of
 * Tillerinobot. All IRC matters have been moved to other classes nowadays.
 */
@Slf4j
public class IRCBot implements GameChatEventConsumer {
    ...
    /**
     * Checks if this is a JAX-RS exception that would have been thrown because an external osu resource is not
     * available.
     */
    ...
    public static boolean isTimeout(Throwable e) {
        return (e instanceof SocketTimeoutException)
                || ((e instanceof IOException)
                        && e.getMessage() != null
                        && (e.getMessage().startsWith("Premature EOF")
                                || e.getMessage()
                                        .startsWith("Remote host terminated the handshake for https://osu.ppy.sh/api/")
                                || e.getMessage().startsWith("Error writing to server for https://osu.ppy.sh/api/")
                                || e.getMessage()
                                        .startsWith("Unexpected end of file from server for https://osu.ppy.sh/api/")
    ...
    /** Queues a response and ensures that the semaphore is released if it is held by the current thread. */
    ...
    private GameChatResponse processPrivateMessage(final PrivateMessage message) throws InterruptedException {
        try {
            try (var _ = PhaseTimer.timeTask("fixId")) {
                GameChatResponse fixIdResponse =
                        new FixIDHandler(resolver).handle(message.getMessage(), null, null, new Default());
                ...
                return userData.usingLanguage((Language lang) -> {
                    ...
                    try {
                        ...
                        String originalMessage =
                                message.getMessage().substring(1).trim();
    ...
    public static final String VERSION_MESSAGE =
            "This is embarrasing. Looks like I didn't know about the HD nerf for a quite a while."
                    + " Don't worry: I'm all caught up now. I'll be updating my pp records over the following days.";

    private GameChatResponse welcomeIfDonator(GameChatEvent user)
            throws SQLException, InterruptedException, IOException, UserException {
        Integer userid;
        try {
        ...
        if (userId != null) {
            ...
            if (apiUser != null) {
                ...
                if (!nick.equals(apiUserIrcName)) {
                    ...
                    log.info(
                            "Detected mapping weirdness. Nick {}; id {}; username {}; api->irc name {}",
                            nick,
                            userId,
                            apiUser.getUserName(),
                            apiUserIrcName);
