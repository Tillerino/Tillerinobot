...
public class OsuTrackHandler extends CommandHandler.WithShorthand {
    ...
    public static GameChatResponse updateResultToResponse(UpdateResult update) {
        if (!update.isExists()) {
            return new Success(String.format(
                    "The user %s can't be found.  Try replaced spaces with underscores and try again.",
                    update.getUsername()));
        }

        if (update.isFirst()) {
            return new Success(
                    String.format("%s is now tracked.  Gain some PP and !update again!", update.getUsername()));
        }

        String mainMessage = String.format(
                Locale.US,
                "Rank: %+d (%+1.2f pp) in %d plays. | View detailed data on [https://ameobea.me/osutrack/user/%s osu!track].",
                update.getPpRank() * -1,
                update.getPpRaw(),
                update.getPlayCount(),
        ...
        if (update.getNewHighscores() != null && !update.getNewHighscores().isEmpty()) {
            ...
            for (Highscore newHighscore : newHighscores) {
                if (count <= 2) {
                    highscoreMessageBuilder.append(String.format(
                            Locale.US,
                            "[https://osu.ppy.sh/b/%d #%d]: %1.2fpp; ",
            ...
            highscoreMessageBuilder.append(String.format(
                    "View your recent hiscores on [https://ameobea.me/osutrack/user/%s osu!track].",
                    update.getUsername()));
