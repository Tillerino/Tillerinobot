...
public class DebugHandler implements CommandHandler {
    ...
    public GameChatResponse handle(String debugCommand, OsuApiUser debugApiUser, UserData debugUserData, Language lang)
        ...
        try {
            CommandHandler commands = CommandHandler.alwaysHandling(
                            "resolve ",
                            (command, apiUser, userData, l) ->
                                    new Success(command + " resolves to " + resolver.resolveIRCName(command)))
            ...
            return new Message("An exception of type " + e.getClass().getSimpleName() + " occurred: " + e.getMessage());
        }
    }
}
