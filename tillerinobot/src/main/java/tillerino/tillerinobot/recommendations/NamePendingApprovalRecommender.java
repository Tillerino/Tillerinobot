...
public class NamePendingApprovalRecommender implements Recommender {
    ...
    public Collection<BareRecommendation> loadRecommendations(
        ...
        List<Play> mappedPlays = topPlays.stream()
                .map(play -> new Play(play.getBeatmapid(), play.getMods(), play.getPp()))
                .toList();
        List<Integer> allExcludes = Stream.concat(
                        exclude.stream(), topPlays.stream().map(TopPlay::getBeatmapid))
                .distinct()
                .toList();
        ...
        return JACKSON.readValue(response.body(), Response.class).recommendations().stream()
                .map(rec -> new BareRecommendation(
                        rec.beatmapId(), rec.mods(), new long[0], (int) rec.pp(), rec.probability()))
                .toList();
    }
    ...
    record Recommendation(
            @BeatmapId int beatmapId, @BitwiseMods long mods, double probability, double pp) {}
}
