...
public abstract class AbstractBeatmapResource implements BeatmapResource {
    public interface BeatmapDownloader {
         ...
         *
         * @param cls A class from the class loader that can load the resources. If you are creating this in a unit test
         *     where the resources are in the same module, getClass() will probably work just fine.
    ...
    public String getFile() {
        try {
            ...
            if (found == null
                    || (!found.getHash().equals(beatmap.getFileMd5())
            ...
            if (!found.getHash().equals(beatmap.getFileMd5())) {
                throw new WebApplicationException(Response.status(Status.BAD_GATEWAY)
                        .entity(format(
                                "Beatmap %s is damaged. Expected hash code: %s Actual: %s",
                                beatmap.getBeatmapId(), beatmap.getFileMd5(), found.getHash()))
                        .build());
            }
