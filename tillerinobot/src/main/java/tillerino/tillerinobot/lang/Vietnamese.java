...
public class Vietnamese extends AbstractMutableLanguage {
	...
	public GameChatResponse welcomeUser(OsuApiUser apiUser, long inactiveTime) {
		if (inactiveTime < 60 * 1000) {
			...
			return new Message(apiUser.getUserName() + "...")
				...
				.then(new Message("Thật tốt khi bạn đã trở lại. Bạn có muốn tôi gợi ý một vài beatmap cho bạn không?"));
		} else {
			String[] messages = {
	...
	public String unknownCommand(String command) {
		return "Lệnh không tồn tại \"" + command
				+ "\". Gõ !help nếu bạn cần giúp đỡ!";
	}
	...
	public GameChatResponse hug(OsuApiUser apiUser) {
		return new Message("Lại đây nào!")
			.then(new Action("/ôm/ " + apiUser.getUserName()));
	}
	...
	public String featureRankRestricted(String feature, int minRank, OsuApiUser user) {
		return "Xin lỗi, hiện tại tính năng " + feature + " chỉ có sẵn đối với những người từ rank " + minRank + " trở lên.";
	}
	...
	public GameChatResponse optionalCommentOnLanguage(OsuApiUser apiUser) {
		 ...
		 */
		return new Message("Đã chuyển sang tiếng Việt. Người dịch:[https://osu.ppy.sh/users/13456818 BCraftMG] FB:[fb.com/xuanhai.tran.184 Xuân Hải Trần]. Xin hãy góp ý thêm về bản dịch.");
	}

	@Override
	public String invalidChoice(String invalid, String choices) {
		return "Xin lỗi, nhưng tùy chọn \"" + invalid
				+ "\" không hợp lệ. Hãy thử " + choices + "!";
	}
