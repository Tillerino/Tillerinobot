...
public abstract class TsundereBase extends AbstractMutableLanguage {
    ...
    public GameChatResponse hug(OsuApiUser apiUser) {
        ...
        recentHugs++;
        int baseLevel = (int) (Math.log(recentHugs) / Math.log(2.236)
                + Math.log(recentRecommendations + 1) / Math.log(5)); // Sum logs base sqrt(5) and 5

        // do some random stuff
        // Ranges from -2 to 10 (but it intentionally never reaches max)
        int hugLevel = (baseLevel < 8 ? baseLevel : 8)
