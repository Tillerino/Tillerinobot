...
public class TsundereGerman extends TsundereBase {
    ...
    protected String getInactiveShortGreeting(String username, long inactiveTime) {
        return welcomeUserShortShuffler.get(
                // "Was soll das hier sein, das Kuckuckspiel?",	//i think the german term is just not known enough, so i
                // will leave it out
                "Das waren nicht mal fünf Minuten...",
                username + " hat heute seinen Verschwindibuszauber perfektioniert." // @Tillerino if you like it, leave
                // it, if not i will just delete this
                // line
                );
    }
    ...
    public String internalException(String marker) {
        return "Huch? Warum funktioniert das nicht? Das war bestimmt dein Fehler!"
                + " Falls dies weiterhin vorkommt, leite folgendes an [https://twitter.com/Tillerinobot @Tillerinobot] oder [http://www.reddit.com/user/tillerino /u/Tillerino] weiter: "
                + marker + ".";
    }

    @Override
    public String externalException(String marker) {
        return "'tschuldige, der osu! Server hat Unsinn gelabert und ich hab lieber ihn anstatt dich geschlagen. Frag einfach nochmal."
                + " Falls der Server weiterhin Unsinn labert, sag [https://twitter.com/Tillerinobot @Tillerinobot] oder [http://www.reddit.com/user/tillerino /u/Tillerino] Bescheid (erwähne "
                + marker + "). Der soll sich darum kümmern!";
    }
    ...
    public String noInformationForModsShort() {
        ...
        return noInformationForModsShortShuffler.get("Diese Mods? Denkste!", "Mods? Welche Mods?", "Nomod liebt dich.");
    }
    ...
    public String unknownCommand(String command) {
        return command
                + "? Ich glaube dir ist nicht ganz klar wer hier der Boss ist. Du machst was ich dir sage, und ich antworte dir, wenn mir danach ist. Benutz !help, wenn dir das zu kompliziert war!";
    }
    ...
    protected GameChatResponse getHugResponseForHugLevel(String username, int hugLevel) {
        return switch (hugLevel) {
            case 0 -> new Action("*Schlägt " + username + "*").then(new Message("'tschuldige, Reflex."));
            case 1 ->
                new Action("*Umarmt " + username + "*")
                        .then(new Message(
                                "Wow, du bist ziemlich schlecht im Umarmen. Jemand sollte dir das mal beibringen."));
            case 2 ->
                new Message("Da ist was auf deinem Rücken, du Chaot. Warte, ich machs eben weg.")
                        .then(new Action("*Umarmt " + username + "*"));
            case 3 ->
                new Action("*Umarmt " + username + "*")
                        .then(
                                new Message(
                                        "I-ich hab nicht versucht dich zu umarmen! Ich hab nur für 'ne Sekunde mein Gleichgewicht verloren und bin auf dich gefallen."));
            case 4 ->
                new Action("*Umarmt " + username + "*")
                        .then(
                                new Message(
                                        "Das Schwerste beim Versuch dich zu umarmen, ist das Loslassen. Ich glaube du schwitzt zu viel."));
            case 5 ->
                new Action("*Schlägt " + username + "*")
                        .then(new Message("Ups... nun, du hast es wahrscheinlich sowieso verdient."));
            case 6 ->
                new Action("*Umarmt " + username + "*")
                        .then(new Message(
                                "Versteh mich nicht falsch, es ist nicht als würde ich dich mögen oder so..."));
            case 7 ->
                new Message("Anhänglichkeit ist was schlechtes, Idiot.").then(new Action("*Umarmt " + username + "*"));
            case 8 ->
                new Message("I-idiot. Es i-ist so als würdest du Spaß daran haben mich zu umarmen.")
                        .then(new Action("*Umarmt " + username + "*"));
            case 9 ->
                new Action("*Umarmt " + username + "*")
                        .then(new Message("Vergiss nicht: du bist für immer hier.")); // Reference to an old meme
            case 10 ->
                new Action("*Schlägt " + username + " hart*")
    ...
    public String featureRankRestricted(String feature, int minRank, OsuApiUser user) {
        return "'tschuldige, " + feature + " ist nur für Leute, die osu! auch spielen können. Rang " + minRank
                + " sollte ansonsten ausreichen, nicht dass du irgendeine Hoffnung hättest diesen jemals zu erreichen.";
    }
    ...
    public GameChatResponse optionalCommentOnNP(OsuApiUser apiUser, BeatmapMeta meta) {
        ...
        if (estimates.getPP(.95) / typicalPP > 2.0) {
            ...
            return new Message(
                    "Diese Map zu spielen wird mich nicht sonderlich beeindrucken... n-n-nicht dass ich das wollen würde.");
        }
        return GameChatResponse.none();
    }
    ...
    public GameChatResponse optionalCommentOnWith(OsuApiUser apiUser, BeatmapMeta meta) {
        ...
        if (estimates.getPP(.95) / typicalPP > 2.0) {
            ...
            return new Message(
                    "Wenn du wie ein Baby behandelt werden willst, hättest du einfach fragen müssen... Nein, fang einfach an zu spielen.");
        }
        return GameChatResponse.none();
    }
    ...
    protected GameChatResponse getOptionalCommentOnRecommendationResponse(int recentRecommendations) {
        return switch (recentRecommendations) {
            case 7 ->
                new Message(
                        "Ich hab viel Freizeit. Ich würde nie Maps raussuchen weil ich dich mag... r-r-rein hypothetisch.");
            case 17 -> new Message("Weißt du, es ist ein Privileg so viel mit mir zu reden, kein Recht.");
            case 37 ->
                new Message(
                        "Wie würdest du eigentlich dieses Spiel spielen, wenn ich dir nicht die ganze Zeit sagen würde wie?");
            case 73 ->
                new Message(
                        "Ich hätte dich schon längst für Belästigung angezeigt, wenn ich dich nicht lieb... Ich hab nichts gesagt.");
            case 173 ->
                new Message(
                        "Kannst mich einfach nicht allein lassen, was? Ich d-denke das ist okay. Aber wag es nicht, das jemandem zu erzählen!");
    ...
    public GameChatResponse optionalCommentOnLanguage(OsuApiUser apiUser) {
        ...
        return new Message(
                optionalCommentOnLanguageShuffler.get(
                        ...
                        // "[https://osu.ppy.sh/u/3135278 MrMorkat] hat mir dabei geholfen Deutsch zu lernen. Das wollte
                        // ich dir immer schon in einer anderen Sprache sagen: Ich lieb... Ich hab nichts gesagt!", I
                        // kinda not like this
    ...
    protected String getInvalidChoiceResponse(String invalid, String choices) {
        return "Was soll \"" + invalid
                + "\" bitte bedeuten!? Falls zwei Finger zu viel sind, versuch doch jeden Buchstaben zu singletappen: "
                + choices;
    }
