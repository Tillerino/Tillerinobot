...
/**
 * This sets up the configuration for production. It is referenced outside of this project.
 *
...
public interface ProdModule {
    ...
    static ProcessorApi sanDoku() {
        return SanDoku.defaultClient(
                URI.create(Optional.ofNullable(System.getenv("SAN_DOKU_URL")).orElse("http://san-doku:8080")));
    }

    @Provides
    static BeatmapDownloader beatmapDownloader() {
        return WebResourceFactory.newResource(
                BeatmapDownloader.class, JerseyClientBuilder.createClient().target("https://osu.ppy.sh"));
    }
    ...
    static @Named("nap.token") String napToken() {
        return env("NAP_TOKEN").orElseThrow(() -> new NoSuchElementException("NAP_TOKEN must be configured!"));
    }

    @Provides
    static @Named("ppaddict.url") String ppaddictUrl() {
        return env("PPADDICT_URL").orElseThrow(() -> new NoSuchElementException("PPADDICT_URL must be configured!"));
    }
