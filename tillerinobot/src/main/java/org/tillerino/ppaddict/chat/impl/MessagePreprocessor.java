...
/**
 * Here we do anything that we can do right after we receive the message and eventually put the message into the event
 * queue. One important job is to ask the {@link Bouncer} if we aren't already processing a message for the user in
 * question. Non-interactive events like joins are passed right through to the message queue.
 */
@Singleton
@Slf4j
...
public class MessagePreprocessor implements GameChatEventConsumer {
    ...
    private GameChatResponse handleSemaphoreInUse(GameChatEvent event) {
        return bouncer.get(event.getNick())
                .map(feedback -> {
                    ...
                    if (processing > 5) {
                        log.warn(
                                "{} - request has been processing for {}. Currently in queue. Event queue size: {} Response queue size: {}",
                                purpose,
                                processing,
                                queue.size(),
                                responses.size());
                    ...
                    if (feedback.getAttemptsSinceEntered() >= 3
                            && !feedback.isWarningSent()
                            && setWarningSent(event, feedback)) {
