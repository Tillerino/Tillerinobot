...
public class Loader<T> implements AutoCloseable {
    ...
    static <T> Loader<T> createLoader(Database conn, Class<? extends T> cls, @NonNull String query, boolean stream)
            throws SQLException {
    ...
    private static <T> Loader<T> getLoaderFullQuery(
        ...
        if (stream) {
            statement = conn.connection()
                    .prepareStatement(fullQuery, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
    ...
    /**
     * @param parameters these must match the query which was used to create this loader. They are set via
     *     {@link PreparedStatement#setObject(int, Object)} and must only be of types that can handled natively by JDBC,
     *     e.g. boxed primitives and String.
     * @return only valid as long as the Loader is not closed. See {@link #queryList(Object...)} for a convenient
     *     alternative.
     */
    @SuppressFBWarnings(
    ...
    public static void setParameters(PreparedStatement preparedStatement, Object... parameters) throws SQLException {
        ...
        if (expectedParameterCount != parameters.length) {
            throw new SQLException(
                    "Expected " + expectedParameterCount + " parameters but received " + parameters.length);
        }
