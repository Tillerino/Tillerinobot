...
/**
 * Wraps a {@link PreparedStatement} to persist Java objects to the database.
 * Depending on the chosen {@link Action}, this can be used to both insert and update rows.
 *
 * <p>
...
public class Persister<T> implements AutoCloseable {
	...
	Persister(Database database, Class<T> cls, Action a) throws SQLException {
		...
		statement = database.prepare(a.command + " `" + mapping.table() + "` (" + mapping.fields() + ") values (" + mapping.questionMarks() + ")");
	}
