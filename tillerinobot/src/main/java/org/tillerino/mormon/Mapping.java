...
/**
 * Utility class that handles mapping between fields and their values in a JDBC {@link PreparedStatement} or
 * {@link ResultSet}.
 *
 * <p>Data can only be mapped for POJOs, i.e. objects with getters and setters as created by e.g. Lombok's {@link Data}
 * annotation. There is currently no support for constructor-based setting of properties or records.
 *
 * <p>Since data is mapped from a {@link ResultSet}, it is possible to map from a relation other than a database table,
 * e.g. a view. Note that the order of the fields of the class at runtime is used to determine the index of the property
 * in the result set / prepared statement. To create suitable statements, use the {@link #fields()} string.
 */
...
public record Mapping<T>(
        ...
        String table,
        List<FieldManager<?>> fieldManagers,
    ...
    private static final Map<Class<?>, TypeHandler<?>> typeHandlers = Map.ofEntries(
            ...
            Map.entry(byte[].class, new TypeHandler<byte[]>() {
                @Override
                public void toStatement(byte[] object, PreparedStatement statement, int parameterIndex)
                        throws SQLException {
    ...
    void set(Object obj, PreparedStatement statement) {
        for (FieldManager<?> fieldManager : fieldManagers) {
            try {
                ...
                throw new ContextedRuntimeException(e).addContextValue("field", fieldManager.field);
            }
        }
    }
    ...
    private record DefaultTypeHandler(int sqlType) implements TypeHandler<Object> {
        @Override
        public void toStatement(Object object, PreparedStatement statement, int parameterIndex) throws SQLException {
