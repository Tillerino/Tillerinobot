...
class WireMockUpdater {
    ...
    record UrlsModule(String wireMockBase) {
        ...
        BeatmapDownloader beatmapDownloader() {
            return WebResourceFactory.newResource(
                    BeatmapDownloader.class, JerseyClientBuilder.createClient().target(wireMockBase));
        }
    }
    ...
    private void updateMocks() throws IOException {
        DaggerWireMockUpdater_UpdateInjector.builder()
                .urlsModule(new WireMockUpdater.UrlsModule(wireMockServer.baseUrl()))
                .build()
                .inject(this);
    ...
    /**
     * Removes all our secrets from the mocks. WireMock offers something similar, but I think it doesn't apply to
     * requests, only to responses.
     */
    ...
    private void checkMocks() throws IOException {
        DaggerWireMockUpdater_CheckInjector.builder()
                .urlsModule(new WireMockUpdater.UrlsModule(wireMockServer.baseUrl()))
                .build()
                .inject(this);
    ...
    private void callRoutes() throws IOException {
        rateLimiter.startSchedulingPermits(
     ...
     * Make sure this runs in the `tillerinobot` module, otherwise the correct paths are not found. Reads credentials
     * for APIs from environment variables: OSUAPIKEY (for v1) OSU_API_CLIENT_ID, OSU_API_CLIENT_SECRET (for v2)
     */
