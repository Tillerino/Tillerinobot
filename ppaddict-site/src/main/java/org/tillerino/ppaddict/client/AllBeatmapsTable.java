...
public class AllBeatmapsTable extends AbstractBeatmapTable implements HasHelpElements {
    ...
    public AllBeatmapsTable(InitialData initialData) {
        ...
        pager = new MyPager((CustomScrollPanel) ((HeaderPanel) table.getWidget()).getContentWidget(), PAGE_SIZE);
    ...
    private Column<Beatmap, SafeHtml> addImageColumn() {
        Header<HideStateAndRankedOnly> f =
                new Header<HideStateAndRankedOnly>(new CompositeCell<>(Arrays.asList(
                        hasCell(
                                ButtonCell::new,
                                (index, object, value) -> object.filterState =
                                        value.equals(HIDING_FILTER) ? SHOWING_FILTER : HIDING_FILTER,
                                object -> object.filterState),
                        renderIfShowingFilter(new HasFillerCell<>("<br />")),
                        hasCell(
                                () -> renderIfShowingFilter(new CheckboxCell()),
                                (index, object, value) -> {
    ...
    private Column<Beatmap, SafeHtml> addNameColumn() {
        Header<Searches> footer = new FilterFooter<>(
                new SearchesCell(new LoggedIn() {
                    @Override
