...
public class MinMaxCell extends CompositeCell<MinMax> {
    ...
    private static List<HasCell<MinMax, ?>> getList(
            final String styleClass, final String min, final String max, final NumberType numberType) {
    ...
    static String intToString(Integer i, String nullString, NumberType numberType) {
        ...
        if (numberType == NumberType.INTEGER) return String.valueOf(i);
        if (numberType == NumberType.DECIMAL) return String.valueOf(i / 100d);
        throw new RuntimeException();
    }

    static Integer stringToInt(String s, String nullString, NumberType numberType) {
        s = s.trim();
        if (s.equals(nullString)) return null;
        try {
            if (numberType == NumberType.DECIMAL) return (int) Math.round(Double.valueOf(s) * 100);
            else if (numberType == NumberType.INTEGER) return Integer.valueOf(s);
            else return Beatmap.minuteColonSecondToSeconds(s);
