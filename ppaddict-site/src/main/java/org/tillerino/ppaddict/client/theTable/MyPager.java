...
public class MyPager extends SimplePager {
  private class MyScrollHandler implements ScrollHandler {
    ...
    public void onScroll(ScrollEvent event) {
      ...
      if (lastLoadFired > System.currentTimeMillis() - 1000)
        return;
      ...
      if (newScrollPos >= maxScrollTop - 1000) {
        ...
        if (display.getVisibleRange().getStart() + display.getVisibleRange().getLength() < display
            .getRowCount()) {
          scrollable.getElement().focus();
          // We are near the end, so increase the page size.
          display.setVisibleRange(display.getVisibleRange().getStart(), display.getVisibleRange()
              .getLength() + MyPager.this.pageSize);
        }
      }
  ...
  public MyPager(final ScrollPanel scrollable, int pageSize) {
    super(TextLocation.CENTER, (SimplePager.Resources) GWT.create(Resources.class), true, 1000,
        true);
