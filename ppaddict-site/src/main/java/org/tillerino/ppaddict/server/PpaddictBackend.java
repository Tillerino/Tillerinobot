...
public interface PpaddictBackend {
    /**
     * A reduced version of {@link OsuApiBeatmap}. The goal is to not have to use the much larger {@link OsuApiBeatmap}
     * which has a lot of fields we don't need and needlessly increases memory usage.
     */
    @Data
    @EqualsAndHashCode
    @ToString
    ...
    /**
     * @return may return null if the server is shutting down or if there are errors in the backend. The map may change
     *     and periodically is replaced entirely. During startup, the map is filled gradually. See also
     *     {@link #getBeatmapsGeneration()}.
     */
    @CheckForNull
    Map<BeatmapWithMods, BeatmapData> getBeatmaps();

    /**
     * @return increased whenever beatmaps are reloaded. This signals that caches need to be cleared. -1 during the
     *     initial load
     */
