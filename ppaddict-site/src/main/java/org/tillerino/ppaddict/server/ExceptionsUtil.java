...
public class ExceptionsUtil {
    ...
    public static PpaddictException getLoggedWrappedException(Logger logger, Throwable exception) {
        ...
        if (exception instanceof IOException) {
            return new PpaddictException(new Default().externalException(IRCBot.logException(exception, logger)));
        }
        return new PpaddictException(new Default().internalException(IRCBot.logException(exception, logger)));
    }
}
